...
::: index
! import callback, ! Host Filesystem Loader
:::
...
local filesystem. The [JavaScript
...
to obtain source code from locations other then the local filesystem
...
1.  **solc / command-line interface**
    ...
    `CLI Path Normalization and Stripping <cli-path-normalization-and-stripping>`{.interpreted-text
...
2.  **Standard JSON**
    ...
    When using the `Standard JSON <compiler-api>`{.interpreted-text
    role="ref"} API (via either the [JavaScript
...
-   `Direct imports <direct-imports>`{.interpreted-text role="ref"},
...
would typically expect in a shell. Any `/./` or `/../` seguments or
...
see them as separate source units. It is the source unit name that
...
1.  First a prefix is computed
    ...
    > -   Prefix is initialized with the source unit name of the
    >     importing source unit.
    > -   The last path segment with preceding slashes is removed from
    >     the prefix.
    > -   Then, the leading part of the normalized import path,
    >     consisting only of `/` and `.` characters is considered. For
    >     every `..` segment found in this part the last path segment
    >     with preceding slashes is removed from the prefix.

2.  Then the prefix is prepended to the normalized import path. If the
    prefix is non-empty, a single slash is inserted between it and the
    import path.
...
-   Every internal `..` segment backtracks one level up in the
    hierarchy.
...
`base path and include paths <base-and-include-paths>`{.interpreted-text
role="ref"}.
:::
...
only issue an error if the source unit name passed to the Host
...
-   Sequences of multiple consecutive path separators are squashed into
    a single separator (unless they are the leading slashes of an [UNC
    path](https://en.wikipedia.org/wiki/Path_(computing)#UNC)).
-   If the path includes a root name (e.g. a drive letter on Windows)
    and the root is the same as the root of the current working
    directory, the root is replaced with `/`.
-   Symbolic links in the path are **not** resolved.
    -   The only exception is the path to the current working directory
        prepended to relative paths in the process of making them
        absolute. On some platforms the working directory is reported
        always with symbolic links resolved so for consistency the
        compiler resolves them everywhere.
-   The original case of the path is preserved even if the filesystem is
    case-insensitive but
    [case-preserving](https://en.wikipedia.org/wiki/Case_preservation)
    and the actual case on disk is different.
...
conversion unambiguous and ensures that the relative path does not start
...
::: index
! allowed paths, ! \--allow-paths, remapping; target
:::
...
-   Outside of Standard JSON mode:
    -   The directories containing input files listed on the command
        line.
...
-   `/home/user/project/token/` (because `token/` contains the input
    file and also because it is the base path),
-   `/lib/` (because `/lib/` is one of the include paths),
-   `/home/user/project/libs/` (because `libs/` is a directory
    containing a remapping target),
...
remapping; prefix, ! remapping; target
:::
...
-   `context` must match the beginning of the source unit name of the
...
it, any modification to the remappings will result in different
bytecode.
...
1.  **Remappings only affect the translation between import paths and
    ...
    `/project/contract.sol` and place it under that exact source unit
    name in the VFS, not under `/contract/contract.sol`.

2.  **Context and prefix must match source unit names, not import
    paths.**

    -   This means that you cannot remap `./` or `../` directly since
        they are replaced during the translation to source unit name but
        you can remap the part of the name they are replaced with:
...
3.  **Target is inserted directly into the source unit name and does not
    necessarily have to be a valid path.**

    -   It can be anything as long as the import callback can handle it.
        In case of the Host Filesystem Loader this includes also
        relative paths. When using the JavaScript interface you can even
        use URLs and abstract identifiers if your callback can handle
        them.

    -   Remapping happens after relative imports have already been
        resolved into source unit names. This means that targets
        starting with `./` and `../` have no special meaning and are
        relative to the base path rather than to the location of the
        source file.

    -   Remapping targets are not normalized so `@root/=./a/b//` will
        remap `@root/contract.sol` to `./a/b//contract.sol` and not
        `a/b/contract.sol`.

    -   If the target does not end with a slash, the compiler will not
...
6.  **Prefix cannot be empty but context and target are optional.**
    ...
    -   If `target` is the empty string, `prefix` is simply removed from
        import paths.
    -   Empty `context` means that the remapping applies to all imports
        in all source units.
